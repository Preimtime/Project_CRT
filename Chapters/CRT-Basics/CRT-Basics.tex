...
\suggestion{you can probably explain it in your own words better -> maybe but I wanted some generally excepted definition (for example this one includes CRTs with multiple electron guns, which I was not aware of)}
...
\end{figure}
\correction{let e- come form cathode and not point source in mid air, cathode could be a bit smaller -> Done}
...
As our CRT uses a hot cathode, more detail on this will be added later along with the description of our cathode's design. \correction{Thomas' correction accepted}
...
The electrons, that leave the electron gun, are still divergent and need to be focused. For our \SI{2}{\kilo\electronvolt} electrons an electrostatic lens is used.  In the simples way these are cylindrically symmetrical pieces of conductor, like a ring or tube, set to an electrical potential. By combining several of them, one can create the same effect as a combination of concave and convex lenses (\cite{Demtroeder3}). \correction{Yes, too strong of a statement actually}
...
The electrostatic lenses are realized using three conducting rings (3), that are set to the same potential but have varying radii: Each consecutive ring has a smaller radius than the previous one. \correction{Thomas' suggestion excepted}
...
\suggestion{Added description for Aquadag as suggested}
Finally the beam hits the phosphorous-coated screen, which fluoresces on electron impact.
